...
export TABLE_NAME=vc-cert-dev PROXY_SECRET=dev-secret \
       ...
       ISSUER_RSA_PRIVATE_KEY_B64=<base64-pkcs8-rsa-key>
...
| `ISSUER_RSA_PRIVATE_KEY_B64` | yes  | -                                 | Base64 PKCS#8 DER RSA key used to sign new credentials        |
| `ISSUER_RSA_KID`         | no       | HTTPS public JWK URL              | Directly dereferenceable `kid` for new VC-JWTs                |
...
1. Generate a new RSA key; publish its public JWK at a new permanent HTTPS URL.
2. Deploy the public JWK before changing `ISSUER_RSA_PRIVATE_KEY_B64` and
   `ISSUER_RSA_KID`. Never remove a public key that signed a credential.
...
- `app/data/challenges.json` is a build-time snapshot of the course (78 slugs
...
- New VC-JWTs use the Open Badges interoperability baseline: RS256, an HTTPS
  JWK `kid`, and `typ: "JWT"`.
