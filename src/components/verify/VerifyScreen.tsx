...
export function VerifyScreen() {
  ...
  useEffect(() => {
    ...
    try {
      island = JSON.parse(
        document.getElementById("__CERT__")?.textContent ?? "null",
      );
    } catch {
      island = null;
    }
    if (island) {
      ...
      else if (island.status === 410)
        setView({ kind: "revoked-stub", id: island.id });
    ...
    case "missing":
      return (
        <StatusPanel tone="red" heading="Certificate not found">
          ...
          <span className="font-mono text-amber">{view.id}</span> exists. Check
          the ID for typos; the characters I, L, O and U are never used.
        </StatusPanel>
      );
    case "revoked-stub":
      return (
        <StatusPanel tone="red" heading="Certificate revoked">
          The certificate{" "}
          <span className="font-mono text-amber">{view.id}</span> existed but
    ...
    case "unreachable":
      return (
        <StatusPanel tone="amber" heading="Could not check right now">
          ...
          <span className="font-mono text-amber">{view.id}</span> could not be
...
function IdLookupForm() {
  ...
  function onSubmit(e: FormEvent) {
    ...
    if (!CREDENTIAL_ID_PATTERN.test(clean)) {
      setError(
        "That does not look like a credential ID (expected VC-GIT-F- plus 8 characters).",
      );
      return;
    }
          ...
          <div className="flex flex-col gap-5 p-6">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <HudLabel tone={valid ? "phos" : "red"} className="text-xs">
                {valid
                  ? "✓ Verified by VersionControl.gr"
                  : "✗ Revoked by the issuer"}
              </HudLabel>
              <span className="hud text-[11px] text-muted">
            ...
            <div>
              <p className="hud pb-2 text-[11px] text-amber">
                Skills demonstrated
              </p>
