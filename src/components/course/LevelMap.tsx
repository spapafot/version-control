      ...
      <main className="mx-auto flex w-full max-w-3xl flex-col gap-4 px-2">
        <div className="mt-4 text-center">
          ...
          <p className="mx-auto mt-2 max-w-md text-sm text-muted">
            {SECTIONS.length} worlds, {ALL_CHALLENGES.length} missions. Work
          ...
          <PixelPanel tone="phos" title="★ Course complete" titleAs="h2">
            <div className="flex flex-col items-start gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm leading-relaxed text-fg">
                All {ALL_CHALLENGES.length} missions cleared. You can issue a
                free certificate with your name on it and share it anywhere.
              </p>
                  ...
                  {list.map((c, i) => {
                    ...
                    return (
                      <li key={c.id} className="flex items-center">
                        {i > 0 && (
                          <span
                            className="mx-0.5 font-mono text-line"
                            aria-hidden
                          >
                            ─
                          </span>
                        ...
                        >
                          {isDone
                            ? "✓"
                            : String(challengeNumber(c.id)).padStart(2, "0")}
                        </Link>
                      </li>
                    );
                  })}
          ...
          <div className="grid grid-cols-1 gap-2 p-4 sm:grid-cols-2">
            {ACHIEVEMENTS.map((a) => {
              ...
              return (
                ...
                >
                  <p
                    className={`hud text-xs ${won ? "glow-text-amber text-amber" : "text-muted"}`}
                  >
                    {won ? "🏅" : "▨"} {a.title}
                  </p>
