...
page.on("response", (r) => {
  if (r.status() === 404) console.error("404 URL:", r.url());
});
...
page.on("console", (m) => {
  ...
  if (OFFLINE_API.test(m.text()) || OFFLINE_API.test(m.location()?.url ?? ""))
    return;
...
try {
  ...
  await page.$$eval("button", (btns) => {
    const b = btns.find(
      (x) => x.textContent?.trim().toUpperCase() === "DECLINE",
    );
    if (b) b.click();
  });
  ...
  await page.goto(`${BASE}/challenge/first-repository/`, {
  ...
  if (pageScrolls)
    fail("challenge page scrolls at 1440x900; notes belong in the dialog");
  ...
  if (!notesText.includes("YOUR OBJECTIVE"))
    fail("lesson notes dialog did not open");
  ...
  const overlay = await page
    .waitForFunction(() => document.body.innerText.includes("COMPLETED"), {
      timeout: 8000,
    })
    .catch(() => null);
  ...
  const cardClears = await page.evaluate(() => {
    ...
    return (
      card.right < term.left || card.left > term.right || card.bottom < term.top
    );
  });
  ...
  const outputStillVisible = await page.evaluate(() =>
    document
      .querySelector(".xterm")
      .innerText.includes("Initialized empty Git repository"),
  );
  if (!outputStillVisible)
    fail("last command output is not readable behind the completion card");
  ...
  const navigated = await page
    .waitForFunction(() => document.body.innerText.includes("MISSION 10"), {
      timeout: 10000,
    })
    .catch(() => null);
  ...
  await page.goto(`${BASE}/challenge/resolve-the-conflict/`, {
    waitUntil: "networkidle0",
  });
  await page.waitForSelector(".xterm textarea", { timeout: 20000 });
  ...
  if (!bodyText.includes("conflict"))
    fail("conflict badge not visible in file explorer");

  await typeInTerminal("cat menu.html");
  const termText = await page.evaluate(
    () => document.querySelector(".xterm").innerText,
  );
  if (!termText.includes("<<<<<<<"))
    fail("conflict markers not shown in terminal cat");
  ...
  if (!mapText.includes("78")) fail("level map missing progress counts");
  ...
  await page.goto(`${BASE}/challenge/lost-commits/`, {
    waitUntil: "networkidle0",
  });
  await page.waitForSelector(".xterm textarea", { timeout: 20000 });
  await new Promise((r) => setTimeout(r, 600));
  await typeInTerminal("git reflog");
  const reflogText = await page.evaluate(
    () => document.querySelector(".xterm").innerText,
  );
  ...
  const disasterDone = await page
    ...
    .catch(() => null);
  if (!disasterDone)
    fail("lost-commits disaster did not complete via reflog recovery");
  await page.screenshot({ path: `${SHOTS}/12-disaster-reflog.png` });

  // ── stash: shelve, handle the interruption, pick it back up ────────
  await page.goto(`${BASE}/challenge/stash-and-switch/`, {
    waitUntil: "networkidle0",
  });
  await page.waitForSelector(".xterm textarea", { timeout: 20000 });
  await new Promise((r) => setTimeout(r, 600));
  ...
  if (!shelved.includes("stash@{0}"))
    fail("stash panel did not show the shelved entry");
  ...
  const stashDone = await page
    ...
    .catch(() => null);
  if (!stashDone)
    fail("stash-and-switch did not complete after stash → switch → pop");
  ...
  if (!mapAfter.includes("4/78"))
    fail(`progress not persisted on map (expected 4/78)`);
  if (!mapAfter.includes("THE TERMINAL"))
    fail("terminal world missing from map");
  if (!mapAfter.includes("GIT DISASTERS"))
    fail("disasters world missing from map");
  ...
  if (!accountText.includes("SIGN IN"))
    fail("account page did not render the sign-in panel");
  if (!accountText.includes("CREATE ACCOUNT"))
    fail("account page missing create-account toggle");
  if (!accountText.includes("GIT CERTIFICATE"))
    fail("account page missing the certificate explainer");
  ...
  if (!quizText.includes("GIT QUIZ"))
    fail("quiz page did not render the hub panel");
  if (!quizText.includes("SPRINT")) fail("quiz hub missing the sprint mode");
  if (!quizText.includes("SET OF 20"))
    fail("quiz hub missing the set-of-20 mode");
  // Difficulty is dealt as a balanced mix, so a player never picks one.
  if (quizText.includes("DIFFICULTY"))
    fail("quiz hub still offers a difficulty picker");
  if (!quizText.includes("LEADERBOARD"))
    fail("quiz hub missing the leaderboard panel");
  ...
  const verifyInput = await page.evaluate(() =>
    Boolean(document.querySelector("#verify-id")),
  );
