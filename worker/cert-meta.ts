...
export function certDescription(p: VerifyPayload): string {
  ...
  return `${p.recipientName} completed all 66 missions of the VersionControl.gr Git course on ${date}. Credential ${p.credentialId} covers branching, merging, conflict resolution and more.`;
}
...
export function slotHtml(p: VerifyPayload): string {
  ...
  return (
    ...
    `the full VersionControl.gr interactive Git course (all 66 missions), on ${esc(date)}. ` +
